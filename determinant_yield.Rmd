...
We propose three dynamic regression models (DRM) to explain relationship between several predictors and Indonesia's local currency bonds (LCB) yield using time series data from 2015 until 2021. The third model that contains 10y-UST yield and its lag-1, 5y-CDS and its lag-1, exchange rate, policy rate and ARIMA errors of AR(1) and AR(2) can perform well in predicting real yield of the LCB in 2021 with small forecast error suggesting satisfying explanatory power of the model. Model statistics shows that 10y-UST and its lag-1 are the main factors that driving the LCB yield.  
...
```{r lcb, out.width= "70%", fig.cap = "10y-LCB yield movement from Jan 2015 until Aug 2021", fig.align='center', fig.pos="H"}
##yield movement
 yield_ts %>%
...
```{r lcbseason, fig.cap = "Seasonal Plot of 10y-LCB Yield", fig.align='center', fig.pos="H"}
...
```{r lcbprob, fig.cap = "Probability distribution of 10y-LCB yield by semester and week in month", warning = FALSE, message = FALSE, fig.width = 12, fig.height = 8, fig.align='center', fig.pos="H"}
# semester yield 10y
yield_ts %>%
  ...
  ylab("10y-LCB yield")+xlab("Week month")+theme_light()
...
```{r usts, out.width= "70%", fig.cap = "10y-UST yield movement from Jan 2015 until Aug 2021", fig.align='center', fig.pos="H"}
##UST yield movement
 yield_ts %>%
...
```{r lcbust, fig.cap = "Timeseries of 10y-LCB yield and 10y-UST yield (adjusted scale)", warning = FALSE, message = FALSE, fig.width = 12, fig.height = 8, fig.align='center', fig.pos="H"}
...
```{r ustseason, fig.cap = "Seasonal plot of 10y-UST yield", fig.align='center', fig.pos="H"}
...
```{r ustprob, fig.cap= "Probability distribution of 10y-UST yield by semester and week in month", message = FALSE, warning = FALSE, fig.width = 12, fig.height = 8, fig.align='center', fig.pos="H"}
# semester ust 10y
yield_ts %>%
...
```{r lcbustcor, fig.cap= "Correlation between 10y-UST yield and 10y-LCB yield", fig.align='center', fig.pos="H"}
yield_factors %>% 
...
```{r forts, out.width= "70%", fig.cap = "Changes in foreign ownership from Jan 2015-Aug 2021", fig.align='center', fig.pos="H"}
yield_foreign %>% 
...
```{r forseason, fig.cap = "Seasonal plot of foreign ownership portion", fig.align='center', fig.pos="H"}
...
```{r forprob, fig.cap="Probability distribution of foreign ownership", message = FALSE, warning = FALSE, fig.width = 12, fig.height = 8, fig.align='center', fig.pos="H"}
# semester 
yield_ts %>% as_tsibble() %>% 
 mutate(foreign_pct = foreign/tot_ownership) %>% 
  ...
  geom_boxplot()+
  facet_wrap(~ year(date) + quarter_semester, scales = "free")+
  ylab("Trillion IDR")+xlab("Week-month")+theme_light()
...
```{r ownprop, out.width= "70%", fig.cap="Total proportion of bonds ownership by industry from Jan 2015-Aug 2021", fig.align='center', fig.pos="H"}
...
```{r lcbfor, fig.cap="Time series of 10y-LCB yield and foreign percentage (adjusted scale)", fig.width = 12, fig.height = 8, fig.align='center', fig.pos="H"}
# LCB and Foreign
yieldts_scale %>%
  ggplot(aes(x = date))+
  geom_line(aes(y = domestic_10y, color = "red") , size = 1)+
  ...
  facet_wrap(~ year(date), scale = "free")+
   scale_color_discrete(name = "Variable", labels = c("Foreign ownership", "10y-LCB yield"))+
  ylab("value")+
  theme_light()+theme(legend.position = "bottom")+scale_x_date(labels = date_format("%b"))
```
...
```{r corfor, fig.cap= "Correlation between foreign ownership and 10y-LCB yield", message = FALSE, warning = FALSE, fig.align='center', fig.pos="H"}
#foreign changes
...
```{r cdsts, out.width= "70%", fig.cap="Indonesia's 5y-CDS from Jan 2015-Aug 2021",fig.pos="H", fig.align='center'}
yield_ts %>% 
  ggplot(aes(x = date, y = cds_5y))+
  geom_line(size = 1, color = "darkgreen")+theme_light()+ylab("5y-CDS")
```

Checking at the annual seasonal plot of the 5y-CDS in figure \@ref(fig:cdsseason), we see that the observed trends are quite the same with the trend of the LCB yield, in which 2015 and 2018 have an increasing trend, whereas 2017 and 2019 have a decreasing trend. 

```{r cdsseason, fig.cap = "Seasonal plot of 5y-CDS", fig.align='center', fig.pos="H"}
...
```{r cdsprob, fig.cap = "Probability distribution of 5y-CDS by semester and week in month", warning = FALSE, message = FALSE, fig.width = 12, fig.height = 8, fig.align='center', fig.pos="H"}
# semester cds 5y
yield_ts %>% 
  ...
  geom_boxplot()+
  facet_wrap(~ year(date) + quarter_semester, scales = "free")+
  ylab("5y-CDS")+xlab("Week-month")+theme_light()
```
...
```{r lcbcds, fig.cap = "Time series of 10y-LCB yield and 5y-CDS (adjusted scale)", fig.width = 12, fig.height = 8, fig.align='center', fig.pos="H"}
# LCB and CDS
yieldts_scale %>% 
  ggplot(aes(x = date))+
  geom_line(aes(y = domestic_10y, color = "red") , size = 1)+
...
```{r corcds, fig.cap= "Correlation between 5y-CDS and 10y-LCB yield", warning = FALSE, message = FALSE, fig.align='center', fig.pos="H"}
###cds vs yield 
yield_ts %>%
...
```{r vixts,out.width= "70%", fig.cap="Volatility index of Indonesia from Jan 2015-Aug 2021", fig.align='center', fig.pos="H"}
yield_ts %>% 
...
```{r vixseason, fig.cap = "Seasonal plot of VIX", fig.align='center',fig.pos="H"}
...
```{r vixprob, fig.cap= "Probability distribution of VIX by semester and week in month", message = FALSE, warning = FALSE, fig.width = 12, fig.height = 8, fig.align='center', fig.pos="H"}
##VIX
# semester 
yield_ts %>% 
 mutate(foreign_pct = foreign/tot_ownership) %>% 
...
```{r lcbvix, fig.cap = "Time series of 10y-LCB yield and VIX (adjusted scale)",fig.width = 12, fig.height = 8, fig.align='center', fig.pos="H"}
# LCB and VIX yield
yieldts_scale %>% 
  ggplot(aes(x = date))+
  geom_line(aes(y = domestic_10y, color = "red") , size = 1)+
...
```{r corvix, fig.cap= "Correlation between VIX and 10y-LCB yield", warning = FALSE, message = FALSE, fig.align='center', fig.pos="H"}
...
```{r rate,out.width= "70%", fig.cap="Changes in policy rate from Jan 2015-Aug 2021", fig.align='center', fig.pos="H"}
yield_ts %>% 
...
```{r corrate, fig.cap= "Correlation between policy rate and 10y-LCB yield", warning = FALSE, message = FALSE, fig.align='center', fig.pos="H"}
#check rate vs yield
yield_ts %>% 
  mutate(year = year(date)) %>% 
  filter(year > 2015) %>% 
...
```{r lcbrate, fig.cap="Time series of 10y-LCB yield and policy rate (adjusted scale)",fig.pos="H", fig.align='center'}
# LCB and Rate yield
yieldts_scale %>% 
...
```{r fxprob, fig.cap = "Probability distribution of exchange rate by semester and week in month", warning = FALSE, message = FALSE, fig.width = 12, fig.height = 8, fig.align='center'}
## Exchange rate
yield_ts %>% 
  ...
  geom_boxplot()+
  facet_wrap(~ year(date) + quarter_semester, scales = "free")+
  ylab("FX rate")+xlab("Week-month")+theme_light()
...
```{r lcbfx, fig.cap="Time series of 10y-LCB yield and exchange rate (adjusted scale)", fig.width = 12, fig.height = 8, fig.align='center'}
# LCB and FX yield
yieldts_scale %>% 
  ggplot(aes(x = date))+
  geom_line(aes(y = domestic_10y, color = "red") , size = 1)+
  ...
  facet_wrap(~ year(date), scale = "free")+
   scale_color_discrete(name = "Variable", labels = c("FX rate", "10y-LCB yield"))+
  ylab("value")+
  theme_light()+theme(legend.position = "bottom")+scale_x_date(labels = date_format("%b"))
```
...
```{r corfx, fig.cap= "Relationship between exchange rate and 10y-LCB yield", fig.align='center', fig.pos="H"}
...
Cointegration test is an important preliminary check before analysing time series of regression model especially in economics and finance, since many variables used in these fields are usually non-stationary, and regressing non-stationary time series could potentially produce spurious results [-@Wharton]. At the same time, if we try to use difference for all non-stationary predictors (detrending), this approach will erase all long run effect of these variables with only short run dynamics remains [-@Wharton]. As a result, the explanatory power of the model will be weakened. 

The concept of cointegration can be described as follows: while individual variable is simply non-stationary, their long-run combination with other variables could potentially be stationary [-@Wharton]. Hence, we will check if all of our time series used in this study are actually cointegrated. 
...
```{r viscoin, fig.cap= "Stationarity of cointegrated variables", fig.align='center', fig.pos='H'}
...
For this purpose, we will select appropriate number of lags for each predictor by comparing the AICc of the predictor's models. The models used in comparison consist of four models, first is a model with only one original predictor against the LCB yield, second is a model with one original predictor and its lag-1 version, third is a model with one original predictor, its lag-1 and lag-2 versions, and fourth is a model with one original predictor, its lag-1 version, lag-2 and lag-3 versions. We also restrict the data for the models as it is important to ensure that the models use same fitting period when comparing number of lags [@Hyndman].
...
We complement the visualization check with a formal test using Ljung-Box test. From the test result, we can confirm that the residuals are similar to white noise since the pvalue produced (with lag = 21) is large (0.24).  
...
```{r modperf, fig.cap="Performance of model in predicting real 10y-LCB yield (blue line is the prediction)", fig.align='center', fig.pos="H", out.width= "80%"}
...
From the plot, the DRM seems to nicely capture the LCB yield's dynamics (trend and volatility) quite well. The gaps between prediction and real yield are mostly small along the forecast horizon, and the real yield is still mostly covered inside the 80% prediction interval band. Another thing to notice here is that the prediction interval looks quite wide. This is because we use information from quite short range time series (5 years long) and at the same time our forecast horizon is quite long (7 months) hence more uncertainty is associated with the prediction. 
