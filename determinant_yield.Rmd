...
US Treasury 10y, Credit Default Swap 5y & 10y, foreign ownership, policy rate (Bank Indonesia 7-day repo rate), exchange rate (Jakarta Interbank Spot Dollar Rate/JISDOR), Issuance Mechanism in Primary Market (new policy of greenshoe auction in 2020)
...
Investors are exposed to gains and losses from exchange rate movements on their holdings of local currency sovereign bond. exchange rate risk can represent an important channel of transmission of market sentiment, uncertainty and default risk to local currency bond yield 
...
#   ggplot(aes(y = domestic_10y)) + 
#   geom_jitter(aes(x = inflation), alpha =0.5)
...
mutate(year = year(date), con_pct = con_bank/tot_ownership, bi_pct = central_bank/tot_ownership, others_pct = others/tot_ownership) %>% 
  ggplot(aes(x = vix, y = domestic_10y)) + 
  geom_point() +
  facet_wrap(~ year)
...
data_model <- yield_factors %>% mutate(year = year(date)) %>% 
